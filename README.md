...
- `LOKI_URL` enables the Grafana Loki log card. Leave it unset to hide live log results.
- `LOKI_BASIC_AUTH_USERNAME` and `LOKI_BASIC_AUTH_PASSWORD` are optional for Loki deployments behind basic auth.
...
- Logs use Loki's `/loki/api/v1/query_range` endpoint and render the newest matching lines first.
...
- Grafana Loki logs: `src/lib/loki-logs.ts`
